...
`)),is:"iron-icon",properties:{icon:{type:String},theme:{type:String},src:{type:String},_meta:{value:Ao.create("iron-meta",{type:"iconset"})}},observers:["_updateIcon(_meta, isAttached)","_updateIcon(theme, isAttached)","_srcChanged(src, isAttached)","_iconChanged(icon, isAttached)"],_DEFAULT_ICONSET:"icons",_iconChanged:function(e){var t=(e||"").split(":");this._iconName=t.pop(),this._iconsetName=t.pop()||this._DEFAULT_ICONSET,this._updateIcon()},_srcChanged:function(e){this._updateIcon()},_usesIconset:function(){return this.icon||!this.src},_updateIcon:function(){this._usesIconset()?(this._img&&this._img.parentNode&&Ba(this.root).removeChild(this._img),""===this._iconName?this._iconset&&this._iconset.removeIcon(this):this._iconsetName&&this._meta&&(this._iconset=this._meta.byKey(this._iconsetName),this._iconset?(this._iconset.applyIcon(this,this._iconName,this.theme),this.unlisten(window,"iron-iconset-added","_updateIcon")):this.listen(window,"iron-iconset-added","_updateIcon"))):(this._iconset&&this._iconset.removeIcon(this),this._img||(this._img=document.createElement("img"),this._img.style.width="100%",this._img.style.height="100%",this._img.draggable=!1),this._img.src=this.src,Ba(this.root).appendChild(this._img))}});const Bo=yo(o||(o=q`
<custom-style>
  <style is="custom-style">
    html {
...
`));Bo.setAttribute("style","display: none;"),document.head.appendChild(Bo.content);const $o=yo(s||(s=q`
<custom-style>
  <style is="custom-style">
    html {
      /*
...
`)),is:"paper-ripple",behaviors:[is],properties:{initialOpacity:{type:Number,value:.25},opacityDecayVelocity:{type:Number,value:.8},recenters:{type:Boolean,value:!1},center:{type:Boolean,value:!1},ripples:{type:Array,value:function(){return[]}},animating:{type:Boolean,readOnly:!0,reflectToAttribute:!0,value:!1},holdDown:{type:Boolean,value:!1,observer:"_holdDownChanged"},noink:{type:Boolean,value:!1},_animating:{type:Boolean},_boundAnimate:{type:Function,value:function(){return this.animate.bind(this)}}},get target(){return this.keyEventTarget},keyBindings:{"enter:keydown":"_onEnterKeydown","space:keydown":"_onSpaceKeydown","space:keyup":"_onSpaceKeyup"},attached:function(){11==Ba(this).parentNode.nodeType?this.keyEventTarget=Ba(this).getOwnerRoot().host:this.keyEventTarget=Ba(this).parentNode;var e=this.keyEventTarget;this.listen(e,"up","uiUpAction"),this.listen(e,"down","uiDownAction")},detached:function(){this.unlisten(this.keyEventTarget,"up","uiUpAction"),this.unlisten(this.keyEventTarget,"down","uiDownAction"),this.keyEventTarget=null},get shouldKeepAnimating(){for(var e=0;e<this.ripples.length;++e)if(!this.ripples[e].isAnimationComplete)return!0;return!1},simulatedRipple:function(){this.downAction(null),this.async((function(){this.upAction()}),1)},uiDownAction:function(e){this.noink||this.downAction(e)},downAction:function(e){this.holdDown&&this.ripples.length>0||(this.addRipple().downAction(e),this._animating||(this._animating=!0,this.animate()))},uiUpAction:function(e){this.noink||this.upAction(e)},upAction:function(e){this.holdDown||(this.ripples.forEach((function(t){t.upAction(e)})),this._animating=!0,this.animate())},onAnimationComplete:function(){this._animating=!1,this.$.background.style.backgroundColor="",this.fire("transitionend")},addRipple:function(){var e=new rs(this);return Ba(this.$.waves).appendChild(e.waveContainer),this.$.background.style.backgroundColor=e.color,this.ripples.push(e),this._setAnimating(!0),e},removeRipple:function(e){var t=this.ripples.indexOf(e);t<0||(this.ripples.splice(t,1),e.remove(),this.ripples.length||this._setAnimating(!1))},animate:function(){if(this._animating){var e,t;for(e=0;e<this.ripples.length;++e)(t=this.ripples[e]).draw(),this.$.background.style.opacity=t.outerOpacity,t.isOpacityFullyDecayed&&!t.isRestingAtMaxRadius&&this.removeRipple(t);this.shouldKeepAnimating||0!==this.ripples.length?window.requestAnimationFrame(this._boundAnimate):this.onAnimationComplete()}},animateRipple:function(){return this.animate()},_onEnterKeydown:function(){this.uiDownAction(),this.async(this.uiUpAction,1)},_onSpaceKeydown:function(){this.uiDownAction()},_onSpaceKeyup:function(){this.uiUpAction()},_holdDownChanged:function(e,t){void 0!==t&&(e?this.downAction():this.upAction())}});const ls={properties:{noink:{type:Boolean,observer:"_noinkChanged"},_rippleContainer:{type:Object}},_buttonStateChanged:function(){this.focused&&this.ensureRipple()},_downHandler:function(e){ns._downHandler.call(this,e),this.pressed&&this.ensureRipple(e)},ensureRipple:function(e){if(!this.hasRipple()){this._ripple=this._createRipple(),this._ripple.noink=this.noink;var t=this._rippleContainer||this.root;if(t&&Ba(t).appendChild(this._ripple),e){var i=Ba(this._rippleContainer||this),n=Ba(e).rootTarget;i.deepContains(n)&&this._ripple.uiDownAction(e)}}},getRipple:function(){return this.ensureRipple(),this._ripple},hasRipple:function(){return Boolean(this._ripple)},_createRipple:function(){return document.createElement("paper-ripple")},_noinkChanged:function(e){this.hasRipple()&&(this._ripple.noink=e)}},hs={observers:["_focusedChanged(receivedFocusFromKeyboard)"],_focusedChanged:function(e){e&&this.ensureRipple(),this.hasRipple()&&(this._ripple.holdDown=e)},_createRipple:function(){var e=ls._createRipple();return e.id="ink",e.setAttribute("center",""),e.classList.add("circle"),e}},cs=[as,Uo,ls,hs];Qa({is:"paper-icon-button",_template:yo(l||(l=q`
    ...
    <iron-icon id="icon" src="[[src]]" icon="[[icon]]"
               alt$="[[alt]]"></iron-icon>
  `)),hostAttributes:{role:"button",tabindex:"0"},behaviors:[cs],registered:function(){this._template.setAttribute("strip-whitespace","")},properties:{src:{type:String},icon:{type:String},alt:{type:String,observer:"_altChanged"}},_altChanged:function(e,t){var i=this.getAttribute("aria-label");i&&t!=i||this.setAttribute("aria-label",e)}});const ds=Qa({_template:yo(h||(h=q`
    <style>
      :host {
        display: inline-block;
        position: fixed;
...
</custom-style>`));fs.setAttribute("style","display: none;"),document.head.appendChild(fs.content);const ms={attached:function(){this.fire("addon-attached")},update:function(e){}};Qa({_template:yo(p||(p=q`
    <style>
      :host {
        display: inline-block;
        float: right;
...
`)),is:"paper-input-char-counter",behaviors:[ms],properties:{_charCounterStr:{type:String,value:"0"}},update:function(e){if(e.inputElement){e.value=e.value||"";var t=e.value.toString().length.toString();e.inputElement.hasAttribute("maxlength")&&(t+="/"+e.inputElement.getAttribute("maxlength")),this._charCounterStr=t}}});const gs=yo(u||(u=q`
<custom-style>
  <style is="custom-style">
    html {
...
`));gs.setAttribute("style","display: none;"),document.head.appendChild(gs.content),Qa({_template:yo(f||(f=q`
    <style>
      :host {
        display: block;
        padding: 8px 0;
...
</iron-iconset-svg>`));document.head.appendChild(dr.content);var pr=$e(R||(R=q`mil-pulse-spinner{--height:100px;--width:100px;--color1:var(--app-primary-color);--color2:gray}#main-content{display:flex;flex-flow:row wrap}#sentinel{width:1px;height:1px;margin-bottom:20px}@media only screen and (max-width:600px){.main-content{flex-flow:column wrap}}@media only screen and (min-width:600px){.main-content{flex-flow:column wrap}}@media only screen and (min-width:768px){.main-content{flex-flow:column wrap}}@media only screen and (min-width:992px){.main-content{flex-flow:row wrap}}@media only screen and (min-width:1200px){.main-content{flex-flow:row wrap}}`));class ur extends Ke{static get properties(){return{countries:{type:Array},filter:{type:String},loaded:{type:Number},isLoading:{type:Boolean},sorter:{type:String}}}constructor(){super(),this.countries=[],this.filter="",this.sorter="",this.loaded=2,this.isLoading=!0}connectedCallback(){super.connectedCallback(),this.getStatistics()}updated(){const e=this.shadowRoot.getElementById("sentinel");if(e){new IntersectionObserver(e=>{e.forEach(e=>{e.intersectionRatio>0&&(this.loaded+=10)})}).observe(e)}}static get styles(){return pr}render(){if(this.isLoading)return ke(F||(F=q`<mil-pulse-spinner id="myMilPulseSpinner"></mil-pulse-spinner>`));let e=this.countries.filter(({country:e})=>-1!==e.toLowerCase().indexOf(this.filter.toLowerCase()));return this.sortCountries(e),e=e||this.countries,e=e.slice(0,this.loaded),ke(D||(D=q` <covid-app-nav @updateSorter="${0}" @keyboardUp="${0}"></covid-app-nav> <div id="main-content"> ${0} </div> `),this.updateSorter,this.updateFilter,this._mapCountries(e))}_mapCountries(e){return ke(B||(B=q`${0}`),e.map((e,t)=>t===this.loaded-1?ke($||($=q`<div id="sentinel"></div>`)):ke(U||(U=q` <covid-stat-box .country="${0}"><covid-stat-box></covid-stat-box></covid-stat-box>`),e)))}getStatistics(){console.log("Fetch COVID-19 Statistics")}updateFilter({detail:{filterValue:e}}){this.filter=e}updateSorter({detail:e}){this.sorter=e.sorterValue}sortCountries(e){e.sort((e,t)=>"Cases"===this.sorter?e.cases.total>t.cases.total?-1:1:"Recovered"===this.sorter?e.cases.recovered>t.cases.recovered?-1:1:"Deaths"===this.sorter?e.deaths.total>t.deaths.total?-1:1:void 0)}}customElements.define("covid-app-body-connected",class extends ur{async getStatistics(){const e=await fetch("https://covid-193.p.rapidapi.com/statistics",{headers:{"x-rapidapi-host":"covid-193.p.rapidapi.com","x-rapidapi-key":"5af96d14dbmsh51fc4214ae8ceb1p1a3d43jsncd88f5576ae6"}}).catch(e=>{console.error(e.message)}),t=await e.json().catch(e=>{console.error(e.message)});this.countries=t.response,this.isLoading=!1}});customElements.define("main-app",class extends Ke{static get styles(){return $e(K||(K=q`:host{--app-primary-color:black;--app-secondary-color:white;--recovered-color:#ACF39D;--deaths-color:#FF5D73;display:block}app-heaeder-layout{background-color:var(--app-secondary-color)}`))}render(){return ke(j||(j=q` <app-header-layout has-scrolling-region fullbleed> <covid-app-body-connected></covid-app-body-connected> </app-header-layout> `))}});
